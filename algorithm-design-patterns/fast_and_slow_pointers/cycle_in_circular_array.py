...
We are given an array containing positive and negative numbers. Suppose the array contains a number
 ‘M’ at a particular index. Now, if ‘M’ is positive we will move forward ‘M’ indices and if ‘M’ is
  negative move backwards ‘M’ indices. You should assume that the array is circular which means two things:
...
def circular_array_loop_exists(arr: List[int]) -> bool:
    ...
    for i in range(len(arr)):
        ...
        while True:
            # move 1 step for slow pointer
            slow = find_next_index(arr, is_forward, slow)
            # move 1 step for fast pointer
...
def test_circular_array_loop_exists(
    arr: List[int], expected: bool,
):
