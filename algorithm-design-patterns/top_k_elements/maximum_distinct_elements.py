...
O(N∗logN) where ‘N’ is the total input numbers. 
...
Since the heap can have a maximum of ‘N/2’ numbers, therefore, extracting 
an element from the heap will take O(logN) and extracting ‘K’ 
numbers will take O(KlogN). So overall, the time complexity of 
our algorithm will be O(N*logN + KlogN).
...
def find_maximum_distinct_elements(nums: List[int], k: int) -> int:
    ...
    min_heap = []
    # insert all numbers with frequency greater than 1 in the min heap
